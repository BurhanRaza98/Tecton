...
struct VolcanoBuilderView: View {
    @StateObject var manager: QuizManager
    @State private var synthesizer = AVSpeechSynthesizer()
    @Environment(\.colorScheme) var colorScheme
    
    init(manager: QuizManager = QuizManager()) {
        // Use explicit StateObject initializer to avoid ambiguity
        self._manager = StateObject<QuizManager>(wrappedValue: manager)
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    ZStack {
                        ...
                        Image("PZV5") // Use the top layer as a ghost outline
                            .resizable()
        ...
        .onAppear {
            ...
            if manager.answeredCorrectly.isEmpty {
                // Preload PZV1 as the base if no image is shown
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    if manager.answeredCorrectly.isEmpty {
                        withAnimation {
                            manager.answeredCorrectly = ["PZV1"]
    ...
    private func questionView(for layer: VolcanoLayer) -> some View {
        VStack(spacing: 16) {
            Text(layer.question)
                .font(.title3)
                .fontWeight(.medium)
    ...
    private var completionView: some View {
        VStack(spacing: 20) {
            ...
            Text("You've successfully built Mount Vesuvius!")
                .font(.title3)
                .multilineTextAlignment(.center)
            
            // Achievement unlock message with custom badge image
            HStack {
                Image("Vesuvius Master")
                    .resizable()
                    .scaledToFit()
                    .frame(width: 40, height: 40)
                Text("Vesuvius Master Achievement Unlocked!")
                    .font(.headline)
            ...
            Text("Go to the Achievements tab and tap on the Vesuvius Master badge to view a 3D model of the volcano!")
                .font(.subheadline)
                .multilineTextAlignment(.center)
            ...
            Button(action: {
                ...
                ProgressManager.shared.markGameCompleted(volcanoName: "Mount Vesuvius", gameType: .volcanoBuilder)
                
                // Switch to Achievements tab (ContentView needs to be updated in a separate step)
...
// Custom button style for a nice scale effect
...
#Preview {
    VolcanoBuilderView(manager: {
        let previewManager = QuizManager()
        previewManager.loadQuiz()
        previewManager.answeredCorrectly = ["PZV1", "PZV2"]
