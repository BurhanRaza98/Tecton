...
struct FujiBuilderView: View {
    @StateObject var manager: FujiQuizManager
    @State private var synthesizer = AVSpeechSynthesizer()
    @Environment(\.colorScheme) var colorScheme
    
    init(manager: FujiQuizManager = FujiQuizManager()) {
        // Use explicit StateObject initializer to avoid ambiguity
        self._manager = StateObject<FujiQuizManager>(wrappedValue: manager)
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    ZStack {
                        // Base background - light green tint for Fuji
                        Color(hex: "#E6F8EF")
                            .frame(height: 400)
                            .cornerRadius(12)
                        
                        // Display a placeholder outline
                        Image("PZF4") // Use the top layer as a ghost outline
                            .resizable()
        ...
        .onAppear {
            ...
            if manager.answeredCorrectly.isEmpty {
                // Preload PZF1 as the base if no image is shown
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    if manager.answeredCorrectly.isEmpty {
                        withAnimation {
                            manager.answeredCorrectly = ["PZF1"]
    ...
    private func questionView(for layer: FujiLayer) -> some View {
        VStack(spacing: 16) {
            Text(layer.question)
                .font(.title3)
                .fontWeight(.medium)
    ...
    private var completionView: some View {
        VStack(spacing: 20) {
            ...
            Text("You've successfully built Mount Fuji!")
                .font(.title3)
                .multilineTextAlignment(.center)
            ...
            HStack {
                ...
                Text("Fuji Master Achievement Unlocked!")
                    .font(.headline)
            ...
            Text("Go to the Achievements tab and tap on the Fuji Master badge to view a 3D model of the volcano!")
                .font(.subheadline)
                .multilineTextAlignment(.center)
            ...
            Button(action: {
                ...
                ProgressManager.shared.markGameCompleted(volcanoName: "Mount Fuji", gameType: .volcanoBuilder)
...
#Preview {
    FujiBuilderView(manager: {
        let previewManager = FujiQuizManager()
        previewManager.loadQuiz()
        previewManager.answeredCorrectly = ["PZF1", "PZF2"]
